...
that it isn't practical to solve them by hand. That's where Python comes in.
...
The limitation for the 1st parameter to be a square matrix is ... quite limiting.
There are problems where you could have 20 constraints and 10 decision variables.
That's where `np.linalg.lstsq()` comes in. Credits to [@halvorsen_2020] for the
workaround. 

```{python}
# Removing the 3rd equation to demonstrate finding solution in rectangular matrix
C = np.array([[1, 2, 3],
              [2, 1, 4]])
              
d = np.array([1250, 1800])

# linalg.lstsq for rectangular matrices
np.linalg.lstsq(C, d, rcond = None)[0]
```
